...
function InfoCard({data}) {
  // console.log("data.item",data?.item?.data?.price_change_percentage_24h?.usd);
  return (
    <div className='border mx-8 lg:mx-2 border-gray-300 rounded-[8px] p-2 xl:p-4  mb-12 lg:mb-0 h-fit'>
      <div className='flex items-center space-x-1.5 lg:space-x-1.5 whitespace-nowrap'>
            <img className='w-3 h-3 md:w-6 md:h-6' src={data?.item?.thumb} alt='trendImage'/>
            <h1>{data?.item?.symbol}</h1>
            <Growth g={data?.item?.data?.price_change_percentage_24h?.usd} />
